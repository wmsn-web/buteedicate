...
class Upgrade_subscription extends CI_controller
{
	...
	public function index($plan_id,$subscr_id)
	{
		$ch = curl_init();
		...
		if ($err) {
		  ...
		  if ($access_token) {
		    // code...
		    return redirect(base_url('Upgrade_subscription/cancel_sub?token='.$access_token.'&plan_id='.$plan_id.'&subscr='.$subscr_id));
		  }
		}
	}
	...
	public function cancel_sub()
	{
		...
		if ($err) {
		  ...
		  echo 'Error:'.$err; 
			...
			return redirect (base_url('membership-plans/subscribe/'.$pln['plan_slug']));
		}
